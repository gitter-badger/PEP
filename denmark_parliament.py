...
def _get_parties(url):
    ...
    for row in table_party:
        ...
        party_entity['fields'].append({'name': 'political_party', 'value': party_name})
        party_entity['fields'].append({'name': 'number_of_members', 'value': quantity})

        party_entities.append(party_entity)

    return party_objects, party_entities
...
def _get_people(party_obj):
    for party in party_obj:
        ...
        for person in table_party:
            ...
            profile_pic = _site_ulr+all_td.find('img')['src'].replace('~', '')
            ...
            people_entity['fields'].append({'name': 'political_party', 'value': party['name']})
            people_entity['fields'].append({'name': 'url', 'value': person_ulr})
            people_entity['fields'].append({'name': 'picture_url', 'value': profile_pic})
            people_entity['fields'].append({'name': 'position', 'value': position})
            people_entity['fields'].append({'name': 'phone_number', 'value': phone})



print _get_people(parties_info)